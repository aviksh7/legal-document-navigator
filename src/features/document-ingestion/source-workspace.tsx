...
export function SourceWorkspace({ document, warnings, onClear }: { document: IngestedSourceDocument; warnings: IngestionWarning[]; onClear: () => void }) {
  ...
  return <section className="provided-workspace" aria-labelledby="provided-heading">
    <div className="provided-heading"><div><span className="eyebrow">Provided document · Source only</span><h2 id="provided-heading" ref={heading} tabIndex={-1}>{document.title}</h2><p className="quiet-label">{isPdf ? `${document.sections.length} physical PDF pages` : "Pasted text · No page numbers"} · {document.sections.reduce((n, s) => n + s.blocks.length, 0)} source blocks</p></div><button type="button" className="button secondary" onClick={onClear}>Clear document</button></div>
    <p className="reading-note">No summary or legal analysis has been generated. Find wording and read it in context. Clearing or reloading removes this document from the application’s active state.</p>
    ...
    <div className="provided-grid"><div className="provided-tools">
      ...
      {selection.reference && <div className="source-footer"><button type="button" className="text-button" onClick={() => trigger.current?.isConnected && trigger.current.focus()}>← Return to search result</button></div>}
    </div></div>
  </section>;
}
